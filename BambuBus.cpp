...
// 使用Preferences库替换Flash操作
Preferences preferences;
const char* NAMESPACE = "BambuBus";
...
bool Bambubus_read() {
  preferences.begin(NAMESPACE, true);  // 只读模式
  if (preferences.getBytesLength("data") == sizeof(data_save)) {
    preferences.getBytes("data", &data_save, sizeof(data_save));
    preferences.end();
    return data_save.check == 0x40614061;
  }
  preferences.end();
  return false;
}
...
void Bambubus_save() {
  ...
  preferences.putBytes("data", &data_save, sizeof(data_save));
  preferences.end();
}

// 获取当前耗材槽编号（返回 BambuBus 当前使用的耗材槽编号）
int get_now_filament_num()
{
  return data_save.BambuBus_now_filament_num;
}
// 获取当前耗材槽灯丝类型（pla、petg、abs等）
...
// 检查指定耗材槽编号是否在线（连接状态）
// 参数 num: 耗材槽编号（0~N）
// 返回 true: 在线（online），false: 离线（offline）
...
// 设置指定耗材槽编号的在线状态
// 参数 num: 耗材槽编号（0~N）
// 参数 if_online: true=设为在线，false=设为离线
...
// 获取指定耗材槽编号的运动状态设置
// 参数 num: 耗材槽编号（0~N）
// 返回 _filament_motion_state_set 枚举值
...
void send_uart(const unsigned char *data, uint16_t length) {
  TX_IDLE = false;
  digitalWrite(BambuBus_pin_de, HIGH);  // 启用发送模式

  BambuBus_uart.write(data, length);
  BambuBus_uart.flush();

  digitalWrite(BambuBus_pin_de, LOW);   // 切换回接收模式
  TX_IDLE = true;
}
...
int get_BambuBus_have_data() {
  return BambuBus_have_data;
}
uint8_t* get_BambuBus_data_buf() {
  return BambuBus_data_buf; // 直接返回数组指针
}
...
// UART接收回调函数
void on_uart_rx() {
  while (BambuBus_uart.available()) {
    unsigned char x = BambuBus_uart.read();
    RX_IRQ(x);
  }
}
...
void BambuBUS_UART_Init(uint32_t baudrate) {
  ...
  digitalWrite(BambuBus_pin_de, LOW); // 默认接收模式

  BambuBus_uart.begin(baudrate, SERIAL_8E1, UART_RX_PIN, UART_TX_PIN);
  BambuBus_uart.onReceive(on_uart_rx);
}
...
int BambuBus_run()
{
  ...
  if (BambuBus_have_data)
  {
    ...
    switch (cmd)
    {
      case BambuBus_package_heartbeat:  // 心跳包 - 用于维持连接检测
        break;
      case BambuBus_package_filament_motion_short:  // 短包耗材运动信息
        send_for_Cxx(buf_X, data_length);  // 发送Cxx类型数据（短包耗材运动）
        break;
      case BambuBus_package_filament_motion_long:  // 长包耗材运动信息
        send_for_Dxx(buf_X, data_length);  // 发送Dxx类型数据（长包耗材运动）
        break;
      case BambuBus_package_online_detect:  // 在线检测
        send_for_Fxx(buf_X, data_length);  // 发送Fxx类型数据（在线检测）
        break;
      case BambuBus_package_REQx6:  // REQx6请求包（暂未实现）
        //send_for_REQx6(buf_X, data_length);
        break;
      case BambuBus_long_package_MC_online:  // MCU在线状态长包
        send_for_long_packge_MC_online(buf_X, data_length);  // 发送MCU在线状态长包
        break;
      case BambuBus_longe_package_filament:  // 耗材信息读取长包
        send_for_long_packge_filament(buf_X, data_length);  // 发送耗材信息长包
        break;
      case BambuBus_long_package_version:  // 版本信息长包
        send_for_long_packge_version(buf_X, data_length);  // 发送版本信息长包
        break;
      case BambuBus_package_NFC_detect:  // NFC检测包（暂未实现）
        // send_for_NFC_detect(buf_X, data_length);
        break;
      case BambuBus_package_set_filament:  // 设置耗材信息
        send_for_Set_filament(buf_X, data_length);  // 发送设置耗材信息命令
        break;
      default:  // 未知命令类型
        break;
    }
    //    switch (cmd) {
    //      case BambuBus_package_heartbeat:  // 0x00 - 心跳包（设备在线状态维持）
    //        Serial.println("[系统] 收到心跳包，连接正常");
    //        break;
    //
    //      case BambuBus_package_filament_motion_short:  // 0x01 - 耗材运动短指令
    //        send_for_Cxx(buf_X, data_length);
    //        Serial.println("[动作] 执行短距耗材运动");
    //        break;
    //
    //      case BambuBus_package_filament_motion_long:  // 0x02 - 耗材运动长指令
    //        send_for_Dxx(buf_X, data_length);
    //        Serial.println("[动作] 执行长距耗材运动");
    //        break;
    //
    //      case BambuBus_package_online_detect:  // 0x03 - 设备在线检测
    //        send_for_Fxx(buf_X, data_length);
    //        Serial.println("[状态] 响应在线检测请求");
    //        break;
    //
    //      case BambuBus_package_REQx6:  // 0x04 - 保留指令（未实现）
    //        Serial.println("[警告] 收到REQx6指令（暂不支持）");
    //        break;
    //
    //      case BambuBus_long_package_MC_online:  // 0x05 - MCU在线状态报告
    //        send_for_long_packge_MC_online(buf_X, data_length);
    //        Serial.println("[状态] 上报MCU在线状态");
    //        break;
    //
    //      case BambuBus_longe_package_filament:  // 0x06 - 耗材信息读取
    //        send_for_long_packge_filament(buf_X, data_length);
    //        Serial.println("[数据] 上报耗材详细信息");
    //        break;
    //
    //      case BambuBus_long_package_version:  // 0x07 - 固件版本查询
    //        send_for_long_packge_version(buf_X, data_length);
    //        Serial.println("[数据] 上报设备版本信息");
    //        break;
    //
    //      case BambuBus_package_NFC_detect:  // 0x08 - NFC检测（未实现）
    //        Serial.println("[警告] 收到NFC检测指令（暂不支持）");
    //        break;
    //
    //      case BambuBus_package_set_filament:  // 0x09 - 耗材参数设置
    //        send_for_Set_filament(buf_X, data_length);
    //        Serial.println("[设置] 处理耗材参数设置");
    //        break;
    //
    //      default:  // 未知指令
    //        Serial.printf("[错误] 收到未知指令类型: 0x%02X\n", cmd);  // 注意保留换行符
    //        break;
    //    }
  }
